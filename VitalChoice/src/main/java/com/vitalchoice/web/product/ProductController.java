...
public class ProductController {
	...
	private static String CURR_THUMB_IMAGE_REPO_PATH = "C:\\git\\2023-Spring-Project-1\\VitalChoice\\src\\main\\webapp\\thumb_img";
	private static String CURR_DETAIL_IMAGE_REPO_PATH = "C:\\git\\2023-Spring-Project-1\\VitalChoice\\src\\main\\webapp\\detail_img";
	@Autowired
	...
	public String product2(MultipartHttpServletRequest request) {
		...
		serv.insertProduct(category_num, product_name, product_price, product_stock, product_desc, product_origin, product_exp);
		...
		for(int i=1; i<=3; i++) {
			...
			try {
				if(mf!=null){
					...
					if(fname!="") {
					serv.insertProductDetailImageFile(i, product_num, fname, product_num+"_"+i);
		...
		for(int j=1; j<=10; j++) {
			...
			try {
				if(mf!=null){
					String fname = mf.getOriginalFilename();
					if(fname!="") {
						serv.insertProductThumbImageFile(j, product_num, fname, product_num+"_"+j);
